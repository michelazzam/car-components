...
function IncreaseStockModal({ productId }: { productId: string }) {
  //---------------------------REFS------------------------------
  ...
  const onSubmit = (data: IncreaseStockSchema) => {
    increaseStock({ amount: data.amount, action: "increase" });
  };

  return (
    <Modal id="increase-stock-modal" size="xs">
      <Modal.Header title="Increase Stock" id="increase-stock-modal" />
      <Modal.Body>
        <form
          ref={formRef}
